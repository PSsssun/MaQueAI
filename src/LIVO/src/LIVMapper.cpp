...
void LIVMapper::handleVIO() {
  ...
  if (fabs((LidarMeasures.last_lio_update_time - _first_lidar_time) - plot_time) <
      (frame_cnt / 2 * 0.1)) {
...
void LIVMapper::handleLIO() {
  ...
  if (pose_output_en) {
    ...
    evoFile << LidarMeasures.last_lio_update_time << " " << _state.pos_end[0] << " "
            << _state.pos_end[1] << " " << _state.pos_end[2] << " " << q.x() << " " << q.y() << " "
            << q.z() << " " << q.w() << std::endl;
  }
  ...
  *pcl_w_wait_pub = *laserCloudWorld;
  ...
  // aver_time_const_H_time = aver_time_const_H_time * (frame_num - 1) / frame_num +
  // solve_const_H_time / frame_num; printf("[ mapping time ]: per scan: propagation %0.6f
  // downsample: %0.6f match: %0.6f solve: %0.6f  ICP: %0.6f  map incre: %0.6f total: %0.6f \n"
  //         "[ mapping time ]: average: icp: %0.6f construct H: %0.6f, total: %0.6f \n",
  //         t_prop - t0, t1 - t_prop, match_time, solve_time, t3 - t1, t5 - t3, t5 - t0,
  //         aver_time_icp, aver_time_const_H_time, aver_time_consu);

  // printf("\033[1;36m[ LIO mapping time ]: current scan: icp: %0.6f secs, map incre: %0.6f secs,
  // total: %0.6f secs.\033[0m\n"
  //         "\033[1;36m[ LIO mapping time ]: average: icp: %0.6f secs, map incre: %0.6f secs,
  //         total: %0.6f secs.\033[0m\n", t2 - t1, t4 - t3, t4 - t0, aver_time_icp,
  //         aver_time_map_inre, aver_time_consu);
...
void LIVMapper::savePCD() {
  if (pcd_save_en &&
      (pcl_wait_save->points.size() > 0 || pcl_wait_save_intensity->points.size() > 0) &&
      pcd_save_interval < 0) {
    ...
    if (img_en) {
      ...
      std::cout << GREEN << "Downsampled point cloud data saved to: " << downsampled_points_dir
                << " with point count after filtering: " << downsampled_cloud->points.size()
                << RESET << std::endl;
      ...
      if (colmap_output_en) {
        ...
        for (size_t i = 0; i < downsampled_cloud->size(); ++i) {
          ...
          fout_points << i << " " << std::fixed << std::setprecision(6) << point.x << " " << point.y
                      << " " << point.z << " " << static_cast<int>(point.r) << " "
                      << static_cast<int>(point.g) << " " << static_cast<int>(point.b) << " " << 0
                      << std::endl;
        }
      }
    } else {
...
void LIVMapper::prop_imu_once(StatesGroup& imu_prop_state,
                              const double dt,
                              V3D acc_avr,
                              V3D angvel_avr) {
  ...
  V3D acc_imu =
      imu_prop_state.rot_end * acc_avr +
      V3D(imu_prop_state.gravity[0], imu_prop_state.gravity[1], imu_prop_state.gravity[2]);

  /* propogation of IMU */
  imu_prop_state.pos_end =
      imu_prop_state.pos_end + imu_prop_state.vel_end * dt + 0.5 * acc_imu * dt * dt;
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver::CustomMsg::ConstPtr& msg_in) {
  ...
  // if ((abs(msg->header.stamp.toSec() - last_timestamp_lidar) > 0.2 && last_timestamp_lidar > 0)
  // || sync_jump_flag)
  // {
...
void LIVMapper::img_cbk(const sensor_msgs::ImageConstPtr& msg_in) {
  ...
  // if ((abs(msg->header.stamp.toSec() - last_timestamp_img) > 0.2 && last_timestamp_img > 0) ||
  // sync_jump_flag)
  // {
    ...
    case ONLY_LIO: {
      ...
      if (!lidar_pushed) {
        ...
        meas.lidar_frame_end_time =
            meas.lidar_frame_beg_time +
            meas.lidar->points.back().curvature / double(1000);  // calc lidar scan end time
        meas.pcl_proc_cur = meas.lidar;
        lidar_pushed = true;  // flag
      }

      if (imu_en &&
          last_timestamp_imu < meas.lidar_frame_end_time) {  // waiting imu message needs to be
...
void LIVMapper::publish_frame_world(const ros::Publisher& pubLaserCloudFullRes,
    ...
    if (pub_num == pub_scan_num) {
      ...
      for (size_t i = 0; i < size; i++) {
        ...
        if (vio_manager->new_frame_->cam_->isInFrame(pc.cast<int>(), 3))  // 100
        {
          ...
          pointRGB.g = pixel[1];
          pointRGB.b = pixel[0];
          // pointRGB.r = pixel[2] * inv_expo; pointRGB.g = pixel[1] * inv_expo; pointRGB.b =
          // pixel[0] * inv_expo; if (pointRGB.r > 255) pointRGB.r = 255; else if (pointRGB.r < 0)
          // pointRGB.r = 0; if (pointRGB.g > 255) pointRGB.g = 255; else if (pointRGB.g < 0)
          // pointRGB.g = 0; if (pointRGB.b > 255) pointRGB.b = 255; else if (pointRGB.b < 0)
          // pointRGB.b = 0;
  ...
  if (pcd_save_en) {
    ...
    if ((pcl_wait_save->size() > 0 || pcl_wait_save_intensity->size() > 0) &&
      ...
      pcd_index++;
      string all_points_dir(string(string(ROOT_DIR) + "Log/PCD/") + to_string(pcd_index) +
                            string(".pcd"));
      pcl::PCDWriter pcd_writer;
      if (pcd_save_en) {
        ...
        if (img_en) {
          pcd_writer.writeBinary(
              all_points_dir,
              *pcl_wait_save);  // pcl::io::savePCDFileASCII(all_points_dir, *pcl_wait_save);
...
void LIVMapper::publish_effect_world(const ros::Publisher& pubLaserCloudEffect,
