    ...
    for (uint i = 0; i < plsize; i++) {
      if ((msg->points[i].line < N_SCANS))  // && ((msg->points[i].tag & 0x30) == 0x10))
      {
        ...
        if (i == 0)
          ...
          // if(fabs(pl_full[i].curvature - pl_full[i - 1].curvature) > 1.0) ROS_ERROR("time jump:
          // %f", fabs(pl_full[i].curvature - pl_full[i - 1].curvature));
...
void Preprocess::Pandar128_handler(const sensor_msgs::PointCloud2::ConstPtr& msg) {
  ...
  // cout << GREEN << "pl_surf.points[0].timestamp: " << pl_surf.points[0].curvature << RESET <<
  // endl; cout << GREEN << "pl_surf.points[1000].timestamp: " << pl_surf.points[1000].curvature <<
  // RESET << endl; cout << GREEN << "pl_surf.points[5000].timestamp: " <<
  // pl_surf.points[5000].curvature << RESET << endl; cout << GREEN <<
  // "pl_surf.points[10000].timestamp: " << pl_surf.points[10000].curvature << RESET << endl; cout
  // << GREEN << "pl_surf.points[20000].timestamp: " << pl_surf.points[20000].curvature << RESET <<
  // endl; cout << GREEN << "pl_surf.points[30000].timestamp: " << pl_surf.points[30000].curvature
...
void Preprocess::robosense_handler(const sensor_msgs::PointCloud2::ConstPtr& msg) {
  ...
  for (int i = 0; i < plsize; ++i) {
    ...
    const bool is_valid =
        (dist_sqr >= blind_sqr) && !std::isnan(x) && !std::isnan(y) && !std::isnan(z);
    if (!is_valid)
      continue;
