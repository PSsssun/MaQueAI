...
void loadVoxelConfig(ros::NodeHandle& nh, VoxelMapConfig& voxel_config) {
  ...
  nh.param<vector<int>>(
      "lio/layer_init_num", voxel_config.layer_init_num_, vector<int>{5, 5, 5, 5, 5});
    ...
    if (plane_ptr_->is_plane_) {
      ...
      if (layer_ < max_layer_) {
        ...
        if (leaves_[leafnum] != nullptr) {
          ...
          leaves_[leafnum] = new VoxelOctoTree(max_layer_,
                                               layer_ + 1,
...
void VoxelMapManager::StateEstimation(StatesGroup& state_propagat) {
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++) {
    ...
    for (int i = 0; i < effct_feat_num_; i++) {
      ...
      // var = state_propagat.rot_end * extR_ * ptpl_list_[i].body_cov_ * (state_propagat.rot_end *
      // extR_).transpose() +
      //       state_propagat.cov.block<3, 3>(3, 3) - point_crossmat * state_propagat.cov.block<3,
      //       3>(0, 0) * point_crossmat;
      ...
      R_inv(i) =
          1.0 / (0.001 + sigma_l + ptpl_list_[i].normal_.transpose() * var * ptpl_list_[i].normal_);
  ...
  // ave_build_residual_time = ave_build_residual_time * (scan_count - 1) / scan_count +
  // build_residual_time / scan_count; ave_ekf_time = ave_ekf_time * (scan_count - 1) / scan_count +
  // ekf_time / scan_count;

  // cout << "[ Mapping ] ekf_time: " << ekf_time << "s, build_residual_time: " <<
  // build_residual_time << "s" << endl; cout << "[ Mapping ] ave_ekf_time: " << ave_ekf_time << "s,
...
void VoxelMapManager::BuildVoxelMap() {
  ...
  for (size_t i = 0; i < feats_down_world_->size(); i++) {
    pointWithVar pv;
    pv.point_w << feats_down_world_->points[i].x, feats_down_world_->points[i].y,
    ...
    if (range_dis <= radius_k * plane.radius_) {
      ...
      if (dis_to_plane < sigma_num * sqrt(sigma_l)) {
        ...
        if (this_prob > prob) {
          ...
          single_ptpl.dis_to_plane_ = plane.normal_(0) * p_w(0) + plane.normal_(1) * p_w(1) +
                                      plane.normal_(2) * p_w(2) + plane.d_;
        }
        return;
      } else {
...
void VoxelMapManager::mapJet(double v,
                             double vmin,
                             double vmax,
                             uint8_t& r,
                             uint8_t& g,
...
void VoxelMapManager::mapSliding() {
  ...
  std::cout << RED << "[DEBUG]: Map sliding using " << t_sliding_end - t_sliding_start << " secs"
            << RESET << "\n";
  return;
}
...
void VoxelMapManager::clearMemOutOfMap(const int& x_max,
  ...
  // std::cout<<RED<<"[DEBUG]: Delete "<<delete_voxel_cout<<" voxels using "<<delete_time<<"
  // s"<<RESET<<"\n";
}