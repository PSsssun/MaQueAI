...
void VIOManager::getWarpMatrixAffineHomography(const vk::AbstractCamera& cam,
                                               const V2D& px_ref,
                                               const V3D& xyz_ref,
...
void VIOManager::getWarpMatrixAffine(const vk::AbstractCamera& cam,
...
void VIOManager::warpAffine(const Matrix2d& A_cur_ref,
...
void VIOManager::retrieveFromVisualSparseMap(
        ...
        if (corre_feat_map != feat_map.end()) {
          ...
          for (int j = 0; j < voxel_num; j++) {
            ...
            if (dir[2] < 0)
              continue;
            dir.normalize();
            // if (dir.dot(norm_vec) <= 0.17) continue; // 0.34 70 degree 0.17 80 degree 0.08 85
            // degree
  ...
  for (int i = 0; i < length; i++) {
    if (grid_num[i] == TYPE_MAP) {
      ...
      // cv::circle(img_cp, cv::Point2f(pc[0], pc[1]), 3, cv::Scalar(0, 0, 255), -1, 8); // Green
      // Sparse Align tracked
      ...
      if (normal_en) {
        ...
        // if (abs(cos_theta) < 0.08) continue; // 0.5 60 degree 0.34 70 degree 0.17 80 degree 0.08
        // 85 degree

        SE3 T_cur_ref = new_frame_->T_f_w_ * ref_ftr->T_f_w_.inverse();

        getWarpMatrixAffineHomography(
            *cam, ref_ftr->px_, pf, norm_vec, T_cur_ref, 0, A_cur_ref_zero);
...
void VIOManager::generateVisualMapPoints(cv::Mat img, vector<pointWithVar>& pg) {
  ...
  for (int i = 0; i < pg.size(); i++) {
    ...
    if (new_frame_->cam_->isInFrame(pc.cast<int>(),
                                    border))  // 20px is the patch size in the matcher
    {
      int index =
...
void VIOManager::updateVisualMapPoints(cv::Mat img) {
  ...
  for (int i = 0; i < total_points; i++) {
    ...
    double delta_theta = (delta_pose.rotation_matrix().trace() > 3.0 - 1e-6)
                             ? 0.0
    ...
    if (add_flag) {
      ...
      Vector3d f = cam->cam2world(pc);
      Feature* ftr_new =
          new Feature(pt, patch_temp, pc, f, new_frame_->T_f_w_, visual_submap->search_levels[i]);
      ftr_new->img_ = img;
      ftr_new->id_ = new_frame_->id_;
      ftr_new->inv_expo_time_ = state->inv_expo_time;
...
void VIOManager::updateReferencePatch(
  ...
  for (int i = 0; i < visual_submap->voxel_points.size(); i++) {
    ...
    if (iter != plane_map.end()) {
      ...
      if (current_octo->plane_ptr_->is_plane_) {
        VoxelPlane& plane = *current_octo->plane_ptr_;
        float dis_to_plane = plane.normal_(0) * p_w(0) + plane.normal_(1) * p_w(1) +
...
void VIOManager::projectPatchFromRefToCur(
  ...
  for (int i = 0; i < visual_submap->voxel_points.size(); i++) {
    ...
    if (pt->is_normal_initialized_) {
      ...
      cv::putText(ref_cur_combine_temp,
                  ref_est,
      ...
      cv::circle(ref_cur_combine_temp,
                 cv::Point2f(pc_prior[0], pc_prior[1]),
                 1,
                 cv::Scalar(255, 255, 255),
                 -1,
                 8);
...
#pragma omp parallel for reduction(+ : error, n_meas)
    ...
    if (error <= last_error) {
      ...
      // K = (H.transpose() / img_point_cov * H + state->cov.inverse()).inverse() * H.transpose() /
      // img_point_cov; auto vec = (*state_propagat) - (*state); G = K*H;
      // (*state) += (-K*z + vec - G*vec);
      ...
      // if ((rot_add.norm() * 57.3f < 0.001f) && (t_add.norm() * 100.0f < 0.001f) &&
      // (expo_add.norm() < 0.001f)) EKF_end = true;
      if ((rot_add.norm() * 57.3f < 0.001f) && (t_add.norm() * 100.0f < 0.001f))
        EKF_end = true;
    } else {
      (*state) = old_state;
...
void VIOManager::plotTrackedPoints() {
  ...
  // {
  //   cv::line(img_cp, cv::Poaint2f(0, grid_size * i), cv::Point2f(img_cp.cols, grid_size * i),
  //   cv::Scalar(255, 255, 255), 1, CV_AA);
  // }
  // for (int i = 0; i < img_cp.cols / grid_size; i++)
  // {
  //   cv::line(img_cp, cv::Point2f(grid_size * i, 0), cv::Point2f(grid_size * i, img_cp.rows),
  //   cv::Scalar(255, 255, 255), 1, CV_AA);
  // }
  // for (int i = 0; i < img_cp.rows / grid_size; i++)
  // {
  //   cv::line(img_cp, cv::Point2f(0, grid_size * i), cv::Point2f(img_cp.cols, grid_size * i),
  //   cv::Scalar(255, 255, 255), 1, CV_AA);
  // }
  // for (int i = 0; i < img_cp.cols / grid_size; i++)
  // {
  ...
  // origin.y = 20;
  // cv::putText(img_cp, text, origin, cv::FONT_HERSHEY_COMPLEX, 0.7, cv::Scalar(0, 255, 0), 2, 8,
  // 0);
}
...
void VIOManager::processFrame(cv::Mat& img,
  ...
  if (width != img.cols || height != img.rows) {
    ...
    cv::resize(img,
               img,
               cv::Size(img.cols * image_resize_factor, img.rows * image_resize_factor),
               0,
               0,
               CV_INTER_LINEAR);
  }
  ...
  // printf("\033[1;32m[ VIO time ]: current frame: retrieveFromVisualSparseMap time: %.6lf
  // secs.\033[0m\n", t2 - t1); printf("\033[1;32m[ VIO time ]: current frame:
  // computeJacobianAndUpdateEKF time: %.6lf secs, comp H: %.6lf secs, ekf: %.6lf secs.\033[0m\n",
  // t3 - t2, computeH, ekf_time); printf("\033[1;32m[ VIO time ]: current frame:
  // generateVisualMapPoints time: %.6lf secs.\033[0m\n", t4 - t3); printf("\033[1;32m[ VIO time ]:
  // current frame: updateVisualMapPoints time: %.6lf secs.\033[0m\n", t6 - t5); printf("\033[1;32m[
  // VIO time ]: current frame: updateReferencePatch time: %.6lf secs.\033[0m\n", t7 - t6);
  // printf("\033[1;32m[ VIO time ]: current total time: %.6lf, average total time: %.6lf
  // secs.\033[0m\n", t7 - t1 - (t5 - t4), ave_total);

  // ave_build_residual_time = ave_build_residual_time * (frame_count - 1) / frame_count + (t2 - t1)
  // / frame_count; ave_ekf_time = ave_ekf_time * (frame_count - 1) / frame_count + (t3 - t2) /
  // frame_count;
  ...
  // origin.x = 20;
  // origin.y = 20;
  // cv::putText(img_cp, text, origin, cv::FONT_HERSHEY_COMPLEX, 0.6, cv::Scalar(255, 255, 255), 1,
  // 8, 0); cv::imwrite("/home/chunran/Desktop/raycasting/" + std::to_string(new_frame_->id_) +
  // ".png", img_cp);
}